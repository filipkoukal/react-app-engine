...
const ChildComponent = ({ handleSubmit }: { handleSubmit: (value: number) => number }) => {
  const [count, setCount] = useState(0);

  return (
    <Stack>
      <Stack>{count}</Stack>
      <Button variant="contained" onClick={() => setCount(handleSubmit(count))}>
        Do action
      </Button>
    </Stack>
  );
      ...
      <Paper sx={{ p: 2, backgroundColor: 'grey.100' }} elevation={2}>
        <Stack gap={2}>
          <Stack gap={2} flexDirection={'column'} justifyContent={'center'}>
            ...
            <Button variant="contained" onClick={() => setUseDecrement(!useDecrement)}>
